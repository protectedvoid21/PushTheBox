...
class LevelSelectState(State):
    ...
    def prepare(self, game_manager, asset_manager):
        ...
        positions = ui_positioner.generate_positions_column(
            x_position=200, y_position=100, button_gap=50, button_count=5, button_width=280, button_height=80
        )

        self._buttons = [
            Button(positions[i],
                   text=f'Level {i + 1}',
                   font=asset_manager.default_font,
                   image=asset_manager[AssetType.GREEN_BUTTON],
                   click_event=lambda x=i: self.select_level(x + 1))
            for i in range(5)
        ]
        ...
        self._buttons += [
            Button((200, int(last_btn_pos.y) + 50, 250, 75),
                   image=self.asset_manager[AssetType.BLUE_BUTTON],
